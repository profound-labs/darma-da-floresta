\chapterAuthor{Ajahn Tan}
\chapterNote{Palestra e sessão de perguntas e respostas dada por Ajahn Tan durante um retiro de meditação.}
\chapter{A mente controlada por ilusões}
\tocChapterNote{Palestra e sessão de perguntas e respostas dada por Ajahn Tan durante um retiro de meditação.}
...
refere ao significado mais profundo, em \emph{páli}. A pessoa que
...
devemos desenvolver a prática de \emph{samādhi}? Porque nossa
mente, se não possuir \emph{samādhi}, é como o mato e o vento que
...
montanha se agitar. A mente de um \emph{anāgāmī} ou
\emph{arahant} também é assim, não se agita com o fluxo das emoções.
Para fazer que a mente tenha \emph{sati}, tenha \emph{samādhi},
nós temos que treinar em focar a mente em \emph{samādhi}, ou seja,
focar \emph{sati} em um objeto de meditação para que a nossa mente
tenha paz. Focar \emph{sati} de maneira contínua numa fundação faz
surgir êxtase, felicidade; surge a equanimidade de \emph{samādhi}.
Se praticar bastante, a mente se unifica, é \emph{ekaggatā arama,
ekaggatā citta}, tem \emph{sati} firme no momento presente. Focar
\emph{sati} em uma fundação de forma contínua faz surgir
\emph{samādhi}; quando \emph{samādhi} surge, gera força para
\emph{sati}, se torna energia para
\emph{sati-paññā}\footnote{Veja nota anterior sobre
“inteligência.”} na nossa mente.

Nossa mente ou \emph{sati}, que vem sendo agitada como o mato, se
perde nas emoções, desejos, raivas, deleites e aversões. Quando
começamos a ter \emph{sati}, a capacidade de lembrar, essa capacidade
...
pessoas. Basta termos \emph{sati} vigiando nossa mente e nós veremos
as emoções, todas as diferentes manifestações da mente surgindo em nós
mesmos. Se perdemos a atenção, se quebramos \emph{sati}, nossa mente
se perde no fluxo das emoções como desejo, raiva e sofrimento que
surgem na mente. Quando perdemos a atenção ou quebramos \emph{sati},
...
Para que \emph{sati-paññā} surja é necessário desenvolver
\emph{samādhi pāramī}. Quando temos a força de \emph{sati},
...
\emph{sati}, focarmos \emph{sati} na mente, vamos ver as emoções e
...
Portanto, se nós ficarmos atentos e tivermos \emph{sati} cuidando
da mente o tempo todo, vigiando a mente, veremos os objetos mentais e
as manifestações da mente. Treinar e desenvolver \emph{samādhi
pāramī} é causa para que \emph{sati} nasça e consiga acompanhar
...
ou realizando alguma atividade, tenhamos \emph{sati} cuidando da
mente, em todas as posturas. Realizando qualquer trabalho, tenha
\emph{sati} focada naquele trabalho. Caso esteja livre de trabalho,
use \emph{sati} para observar sua própria mente, faça sua mente ficar
...
\emph{sati-paññā} tem que enxergar a impermanência, largar os
objetos mentais, e então a mente fica em equanimidade. Mas se
\emph{sati-paññā} não tiver força suficiente, então focamos
\emph{sati} no objeto de meditação por três ou cinco minutos e o
pensamento ruim some da nossa mente.

Focar \emph{sati} na respiração, ou no mantra “\emph{Buddho}”, é
possível em qualquer postura, quer seja de pé, andando, sentado ou
deitado – é possível focar \emph{sati}, desenvolver
\emph{samādhi} da mesma forma. Mas quando desejamos desenvolver
\emph{sati} e \emph{samādhi} de forma contínua, procuramos a
...
usar \emph{sati} para olhar a mente, veremos todas as formas de
...
Se tivermos \emph{sati} e virmos os pensamentos sobre o passado,
usamos inteligência para refletir: “O passado já se foi, para que
pensar nisso?” Traga \emph{sati} de volta ao momento presente ou de
...
\emph{sati} de volta à respiração ou ao mantra “\emph{Buddho,
...
para que me preocupar?” e traga \emph{sati} de volta ao momento
presente.

Experimentem usar uma técnica de meditação chamada
\emph{maranasati}. Por exemplo, hoje a noite iremos dormir às onze ou
à meia-noite. Use \emph{maranasati} para imaginar que às onze ou à
meia-noite morreremos. Às vezes essa técnica de \emph{maranasati} é
...
focar \emph{sati} e desenvolver \emph{samādhi} continuamente:
desde o momento em que acordar e abrir os olhos, estabeleça
\emph{sati} em sua mente. Quer esteja fazendo um trabalho sozinho ou
em grupo, tenha \emph{sati} naquela atividade. Quando tiver tempo
...
sorte tive \emph{sati} e consegui me perguntar: “Eh? Esta é a minha
mente, por que não consigo controlá-la?” Eu tive \emph{sati} para ver
...
No \emph{uposatha}, ou quando tinha tempo livre, ele ia até o altar
...
Então sentei em meditação e foquei \emph{sati} na ponta do nariz:
...
Deve ter sido sorte minha que, quando foquei \emph{sati} na
respiração, minha mente parou de pensar em todas aquelas histórias.
Tinha \emph{sati} no momento presente, tinha \emph{sati} na
respiração, tinha \emph{sati} na meditação sobre a respiração de
...
postura e saí da meditação, tinha \emph{sati} firme no momento
presente, a mente estava unificada.

Eu vi o resultado de praticar meditação. Quando saí da postura de
meditação sentada, \emph{sati} era capaz de controlar a mente. Quando
...
\emph{samādhi} é causa para que haja \emph{sati}, e quando há
\emph{sati} somos capazes de controlar nossa mente. Desde então,
...
minutos. Então fiquei sabendo que, para ter \emph{sati} controlando a
mente, é necessário praticar \emph{samādhi}. Depois disso eu passei
...
houvesse um pensamento ruim, \emph{sati} via aquele pensamento e eu
...
aquilo, eu simplesmente trazia \emph{sati} de volta à respiração e o
...
observar \emph{sīla} e desenvolver \emph{samādhi}, usem
\emph{sati} para cuidar de sua mente de forma equilibrada, não
tensionem demais para que não surja estresse, nem afrouxem demais para
que não percam \emph{sati}. A prática não é feita para gerar
...
consegue aguentar, mude de posição. Mas se focou \emph{sati} e
desenvolveu \emph{samādhi}, se a mente se pacificou primeiro e em
seguida surgiu a dor, por exemplo dor na perna, existindo um pouco de
\emph{sati} e \emph{samādhi}, \emph{sati} vai focar naquela
...
Procure separar a mente do corpo utilizando \emph{sati-paññā};
...
use \emph{sati-paññā} para contemplar e buscar a verdade sobre a
...
desaparecer. Procurem usar \emph{sati-paññā} para contemplar o
corpo no corpo e separar a mente do corpo. Usem \emph{sati-paññā
...
\emph{sati-paññā} tenha força para contemplar o corpo no corpo, as
sensações nas sensações, tem que haver \emph{samādhi} unido à
\emph{sati-paññā}. Se \emph{samādhi} ainda não surgiu, se nós
...
sua mente através de \emph{sati}, porque você disse estava tendo
muitos pensamentos em sua cabeça, então percebeu, por ter
\emph{sati}. Então você foi a um quarto e então descobriu, ou soube,
...
superar esse sofrimento? \emph{Sādhu}.
...
Mas o sofrimento que eu vi de verdade\ldots{} Eu ainda não tinha tido
...
-- Quando estiver perto da morte tenha \emph{sati} focada no
momento presente. Ainda estando vivo, foque \emph{sati} e desenvolva
\emph{samādhi} para que a mente se pacifique. Quando a mente
...
contemplado, deixe a mente descansar em \emph{samādhi}.
...
-- Se ela tiver \emph{sati} ela vai conseguir, e o carma que eu
...
de iluminação, não é um A\emph{ryia Puggala}, quer seja dos níveis
...
-- Tem que determinar o \emph{pāramī}. Se não quiser receber
o resultado do mérito que leva a nascer como anjos ou deuses\ldots{}
não é? Tem que determinar o \emph{pāramī}. Fizemos mérito
frequentemente e isso é o que gera resultado, mas nós temos que
determinar o \emph{pāramī}: “Eu desejo nascer humano, encontrar
o ensinamento do Buda e viver num país onde o budismo prospera.” Temos
que determinar o \emph{pāramī}. Se acumulamos mérito o
suficiente, determinar o \emph{pāramī} conseguirá que as coisas
